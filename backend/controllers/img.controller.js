...
exports.createImg = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .send({ message: "Successfully created the image" });
    } catch (error) {
...
exports.updateImage = async (req, res) => {
    try {
        ...
        if (!img) {
            return res.status(400).send({ message: "There is no images" });
        }
        return res
            .status(200)
            .send({ message: "successfully updated images", data: img });
    } catch (error) {
        return res.status(500).send({ message: `${error.message}` });
    }
};
exports.deleteImage = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .send({ message: "The image deleted successfully" });
    } catch (error) {
        return res.status(500).send({ message: `${error.message}` });
    }
};
